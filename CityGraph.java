...
/**
 * This class changes the functionality of the CS400Graph class to work over an undirected graph
 * instead of a directed graph by copying directed edges over into their inverses, effectively making
 * undirected edges.
 */
public class CityGraph<NodeType, EdgeType extends Number> implements ICityGraph<NodeType, EdgeType> {
    ...
    public boolean insertEdge(NodeType source, NodeType target, EdgeType weight) {
        ...
        for (Edge e : targetVertex.edgesLeaving) {
            if (e.target == sourceVertex) {
                if (e.weight.doubleValue() == weight.doubleValue()) {
                                       // do not return yet if edge already exists
                } else {
                    e.weight = weight; // otherwise update inverse edge weight
                                       // but dont yet return, have yet to update the forward edge
                }
            }
        }
        ...
        // also add inverse edge (makes it undirected) 
    ...
    public boolean removeEdge(NodeType source, NodeType target) {
        ...
        if (removeEdge != null) { // remove edge that is successfully found
            ...
            // do not return yet, have to find reverse edge
        }

        // find reverse edge to remove
        removeEdge = null;
        for (Edge e : targetVertex.edgesLeaving)
            if (e.target == sourceVertex)
                removeEdge = e;
        if (removeEdge != null) { // remove edge that is successfully found
            targetVertex.edgesLeaving.remove(removeEdge);
            return true;
        }
    ...
    public int getEdgeCount() {
        ...
        return edgeCount / 2; // we divide by two, 
                              // since there is now a hidden inverse edge
                              // as a byproduct of converting to undirected
    }
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            dataSequence = new LinkedList<>(copyPath.dataSequence);
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) throws NoSuchElementException {
        if (!(vertices.containsKey(start) && vertices.containsKey(end))) {
            throw new NoSuchElementException("shortestPath() encountered an exception: " +
                    "Either the start node or the end node could not be found.");
        }

        // initialize a priority queue of paths,
        // and a hashtable holding the visited nodes
        PriorityQueue<Path> paths = new PriorityQueue<>();
        Hashtable<NodeType, Path> visited = new Hashtable<>();

        // initialize the starting vertex and starting path
        // putting these values in the queue and hashtable
        Vertex startVertex = vertices.get(start);
        Path startPath = new Path(startVertex);
        paths.add(startPath);
        visited.put(startVertex.data, startPath);

        while (!paths.isEmpty()) {
            Path path = paths.remove();

            // if the start and ends of this path is a match, return this path
            if (path.start.data.equals(start) && path.end.data.equals(end)) {
                return path;
            }

            // if there are more edges, add them to a list of edges
            Vertex vertex = vertices.get(path.end.data);
            LinkedList<Edge> edges = new LinkedList<>();
            if (vertex != null && vertex.edgesLeaving != null) {
                edges = vertex.edgesLeaving;
            }

            // loop through all edges for cheapest
            for (Edge edge : edges) {
                Path nextPath = new Path(path, edge);

                // if the vertex has not been visited, add to the queue and hashtable
                if (!visited.containsKey(edge.target.data)) {
                    paths.add(nextPath);
                    visited.put(edge.target.data, nextPath);

                    // otherwise, if the node has been visited, check that this path is
                    // cheaper, and update the queue and hashtable
                } else if (nextPath.distance < visited.get(edge.target.data).distance) {
                    paths.add(nextPath);
                    visited.remove(edge);
                    visited.put(edge.target.data, nextPath);
                }
        ...
        throw new NoSuchElementException("shortestPath() encountered an exception: No such path found.");
    }
     ...
     *                                end can be found
     */
    public List<NodeType> shortestPath(NodeType start, NodeType end) throws NoSuchElementException {
